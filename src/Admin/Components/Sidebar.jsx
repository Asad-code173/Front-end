...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`bg-white h-screen fixed top-0 left-0 w-44 sm:w-52 z-40 transition-transform duration-300 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-64"
        } sm:translate-x-0`}
      >
        <h2 className="text-2xl font-bold mb-8 ml-12 mt-4">Logo.</h2>
        ...
        {sideItems.map((item) => (
          <Link
            ...
            className={`flex items-center ml-3 space-x-3 py-2 px-4 transition ${
              location.pathname === `/${item.url}` ? 'bg-blue-400 font-bold' : 'hover:bg-blue-100'
            }`}
          >
